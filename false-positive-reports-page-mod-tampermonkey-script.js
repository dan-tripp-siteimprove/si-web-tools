...
(async function (scriptVars) {
	...
	*/
    ...
    function parseCsv(csvContents_) {
        let lines = csvContents_.split('\n').filter(line => line.trim() !== '');
        let rRows = [];
        for (let i = 0; i < lines.length; i++) {
            let curRow = lines[i].split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/);
            // ^^ This regex handles commas inside quotes
            curRow = curRow.map(x => x.trim().replace(/^"|"$/g, ''))
                .map(x => x.replace(/\\n/g, '\n'));
            if(rRows.length == 0 && curRow[0] !== 'Account ID') {
                throw new Error('error parsing CSV spreadsheet (header not found)');
            }
            rRows.push(curRow);
        }
        return rRows;
